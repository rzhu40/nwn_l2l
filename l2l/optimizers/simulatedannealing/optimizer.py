...
class SimulatedAnnealingOptimizer(Optimizer):
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        logger.info("  Evaluated %d individuals", len(temp_list))
        logger.info('  Average Fitness: %.4f', temp_list.mean())
        logger.info("The best individual was %s with fitness %s", best_indiv, best_fitness)

        generation_result_dict = {
            'generation': self.g,
            'current_fitness': weighted_fitness.item(),
            'best_fitness_in_run': temp_list.max(),
            'average_fitness_in_run': temp_list.mean(),
        }
