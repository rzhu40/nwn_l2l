...
def generate_dist_centroids(number_of_wires, loc, scale, beta=5):
    ...
    xc = np.random.rand(number_of_wires) * loc # uniform random in [0,Lx) for each coordinate dimension
    yc = np.random.rand(number_of_wires) * loc # uniform random in [0,Lx) for each coordinate dimension
...
def export_to_matlab(wires_dict, filename=None, save_pkl=False):
    ...
    # Save to mat format
    import os
    from edamame.utils import root_path
    pathfile = os.path.join(root_path, 'connectivity_data', filename)
